...
public class TestGetNewRoundCategoriesUseCase
{
    ...
    {
        IGetNewRoundCategoriesUseCase useCase = new GetNewRoundCategoriesUseCase(new CategoriesReadOnlyRepositoryJson());

        CategoryDto[] actualResult = useCase.GetRandomCategories(Constants.Categories.CategoriesPerRound);
        Assert.IsTrue(new CategoriesReadOnlyRepositoryJson().Exists(actualResult.Select(c => c.Name).ToArray()));
        int cycles = 100;
